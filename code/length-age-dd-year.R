...
# list params
params <- c("intercept", "beta_dd", "beta_age", "v", "tau")
pars <- c("intercept", "beta_dd", "beta_age", "tau")
...
#
# saveRDS(jags_fit, here::here("outputs", "fits", "length-age-dd-year-jags.Rds"))
jags_fit <- readRDS(here::here("outputs", "fits", "length-age-dd-year-jags.Rds"))
...
traceplot_main
png(here::here("outputs", "plots", "length-age-dd-year",
               "traceplot_main.png"),
...
fit_func <- function(dat, coefs){
  coefs["intercept"] + coefs[paste0("v[", dat$spawn_year_index, "]")] + dat$dd * coefs["beta_dd"] + dat$Age * coefs["beta_age"]
}

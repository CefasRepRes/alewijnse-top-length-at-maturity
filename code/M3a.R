...
params <- c("intercept", "beta_sex", "beta_age1",
            "beta_age2", "beta_dd", "delta", "gamma", "tau")
pars <- c("intercept", "beta_sex[1]", "beta_sex[2]", "beta_age1",
          "beta_age2", "beta_dd", "delta[1]", "delta[2]", "gamma", "tau")
...
#                                   n.thin = 1);beepr::beep()
#
# saveRDS(jags_fit, here::here("outputs", "fits", "M3a-jags.Rds"))
jags_fit <- readRDS(here::here("outputs", "fits", "M3a-jags.Rds"))
...
traceplot
png(here::here("outputs", "plots", "M3a",
               "traceplot.png"),
...
fit_func <- function(dat, coefs){
  coefs["intercept"] + coefs[paste0("beta_sex[", dat$Sex, "]")] + coefs["beta_dd"] * dat$dd +
...
for (i in 1:n_sims) {
  ...
  sigma <- rep(1 / sqrt(coefs_i["tau"]), length(lp))
...
# modify fit function
fit_func <- function(dat, coefs){
  coefs["intercept"] + coefs[paste0("beta_sex[", dat$Sex, "]")] + coefs["beta_dd"] * dat$dd_scaled +
...
fit_sigma <- function(dat, coefs){
  rep(1 / sqrt(coefs["tau"]), length(lp))
}

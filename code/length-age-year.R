...
# list params
params <- c("intercept", "beta_age", "v", "tau")
pars <- c("intercept", "beta_age", "tau")
...
#
# saveRDS(jags_fit, here::here("outputs", "fits", "length-age-year-jags.Rds"))
jags_fit <- readRDS(here::here("outputs", "fits", "length-age-year-jags.Rds"))
...
traceplot_main
png(here::here("outputs", "plots", "length-age-year",
               "traceplot_main.png"),
...
fit_func <- function(dat, coefs){
  coefs["intercept"] + coefs[paste0("v[", dat$spawn_year_index, "]")] + dat$Age * coefs["beta_age"]
}

...
# list params
params <- c("intercept", "beta_sex", "tau")
pars <- c("intercept", "beta_sex[1]", "beta_sex[2]", "tau")

mod_dat <- with(dd_dat, list(sex = Sex,
                             length = Length,
                             n = nrow(dd_dat)))
str(mod_dat)
...
#
# saveRDS(jags_fit, here::here("outputs", "fits", "length-sex-jags.Rds"))
jags_fit <- readRDS(here::here("outputs", "fits", "length-sex-jags.Rds"))

...
pars_delta <- c("delta_sex[1]", "delta_sex[2]",
                paste0("delta_year[", 1:length(unique(dd_dat$spawn_year)), "]"))
...
jags_fit <- readRDS(here::here("outputs", "fits", "length-age-dd-sex-year-breakpoint-jags.Rds"))
...
fit_func <- function(dat, coefs){
    coefs["intercept"] + coefs[paste0("v[", dat$spawn_year_index, "]")] + coefs[paste0("beta_sex[", dat$Sex, "]")] + coefs["beta_dd1"] * (dat$dd - (coefs[paste0("delta_sex[", dat$Sex, "]")] + coefs[paste0("delta_year[", dat$spawn_year_index, "]")])) + coefs["beta_dd2"] * sqrt((dat$dd - (coefs[paste0("delta_sex[", dat$Sex, "]")] + coefs[paste0("delta_year[", dat$spawn_year_index, "]")]))^2 + coefs["gamma"]) + dat$Age * coefs["beta_age"]
}
...
deltas$delta <- coefs_mean[paste0("delta_sex[", deltas$Sex, "]")] + coefs_mean[paste0("delta_year[", deltas$spawn_year_index, "]")]
deltas$delta_up <- coefs_up[paste0("delta_sex[", deltas$Sex, "]")] + coefs_up[paste0("delta_year[", deltas$spawn_year_index, "]")]
deltas$delta_low <- coefs_low[paste0("delta_sex[", deltas$Sex, "]")] + coefs_low[paste0("delta_year[", deltas$spawn_year_index, "]")]

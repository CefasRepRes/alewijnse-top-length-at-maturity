...
//    b1a + b1b + b2 + b4 ~ 1,
  ...
  transformed parameters {
    ...
    for (n in 1:N) {
      // compute omega
      omega = omega_0 + omega_recap[recap_year[n]];

      // linear predictor
      if (length[n] <= omega) {
        mu[n] = alpha + alpha_iid[tag_year[n]] + b1a * length[n] + b2 * sex[n] + b4 * year[n];
      } else {
        mu[n] = alpha + alpha_iid[tag_year[n]] + b1a * omega + b1b * (length[n] - omega) + b2 * sex[n] + b4 * year[n];
      }
    }
  }
  ...
  generated quantities {
    ...
    for (n in 1:N) {
      if (length[n] <= omega) {
        log_lik[n] = normal_lpdf(growth[n] | alpha + alpha_iid[tag_year[n]] + b1a * length[n] + b2 * sex[n] + b4 * year[n], sigma);
      } else {
        log_lik[n] = normal_lpdf(growth[n] | alpha + alpha_iid[tag_year[n]] + b1a * omega + b1b * (length[n] - omega) + b2 * sex[n] + b4 * year[n], sigma);
      }
    }
  }

...
//      ((b1a * omega + b1b * (length - omega)) * step(length - omega)) + b2 * sex,
//    alpha ~ 1 + (1 | tag_year),
//    omega ~ 1 + omega_temp * temp
//    b1a + b1b ~ 1,
  ...
  parameters {
    ...
    real omega_temp; // omega with temp
  ...
  transformed parameters {
    ...
    for (n in 1:N) {
      // compute omega
      omega = omega_0 + omega_temp * temperature[n];

      // linear predictor
      if (length[n] <= omega) {
        mu[n] = alpha + alpha_iid[tag_year[n]] + b1a * length[n] + b2 * sex[n];
      } else {
        mu[n] = alpha + alpha_iid[tag_year[n]] + b1a * omega + b1b * (length[n] - omega) + b2 * sex[n];
      }
    }
  }
  ...
  generated quantities {
    ...
    for (n in 1:N) {
      if (length[n] <= omega) {
        log_lik[n] = normal_lpdf(growth[n] | alpha + alpha_iid[tag_year[n]] + b1a * length[n] + b2 * sex[n], sigma);
      } else {
        log_lik[n] = normal_lpdf(growth[n] | alpha + alpha_iid[tag_year[n]] + b1a * omega + b1b * (length[n] - omega) + b2 * sex[n], sigma);
      }
    }
  }

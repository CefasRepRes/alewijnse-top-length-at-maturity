...
//    b1a + b1b + b2 + b4 ~ 1,
  ...
  parameters {
    ...
    real omega_temp; // effect of temperature
    real b1a;  // slope before omega
    real b1b;  // slope after omega
    real b4;  // effect of trend (year) on growth
  }
  ...
  transformed parameters {
    ...
    for (n in 1:N) {
      // compute omega
      omega = omega_0 + omega_sex * sex[n] + omega_temp * temperature[n];

      // linear predictor
      if (length[n] <= omega) {
        mu[n] = alpha + alpha_iid[tag_year[n]] + b1a * length[n] + b4 * year[n];
      } else {
        mu[n] = alpha + alpha_iid[tag_year[n]] + b1a * omega + b1b * (length[n] - omega) + b4 * year[n];
      }
    }
  }
  ...
  model {
    ...
    omega_temp_sd ~ cauchy(0, 1); // prior
    omega_temp ~ normal(0, omega_temp_sd); // prior
  ...
  generated quantities {
    ...
    for (n in 1:N) {
      if (length[n] <= omega) {
        log_lik[n] = normal_lpdf(growth[n] | alpha + alpha_iid[tag_year[n]] + b1a * length[n] + b4 * year[n], sigma);
      } else {
        log_lik[n] = normal_lpdf(growth[n] | alpha + alpha_iid[tag_year[n]] + b1a * omega + b1b * (length[n] - omega) + b4 * year[n], sigma);
      }
    }
  }
